...
class PenggunaController extends SecureController{
	...
	function index($fieldname = null , $fieldvalue = null){
		...
		if(!empty($request->search)){
			...
			$search_condition = "(
				pengguna.id_Pengguna LIKE ? OR 
				pengguna.username LIKE ? OR 
				pengguna.nama LIKE ? OR 
				pengguna.Jabatan LIKE ? OR 
		...
		if(!empty($request->orderby)){
			...
			$db->orderBy("pengguna.id_Pengguna", ORDER_TYPE);
		}
		if($fieldname){
	...
	function view($rec_id = null, $value = null){
		...
		if($value){
			...
			$db->where("pengguna.id_Pengguna", $rec_id);; //select record based on primary key
		}
	...
	function add($formdata = null){
		if($formdata){
			...
			$request = $this->request;
			//fillable fields
			$fields = $this->fields = array("username","nama","Jabatan","email","password","photo","user_role_id");
	...
	function delete($rec_id = null){
		...
		$db->where("pengguna.id_Pengguna", $arr_rec_id, "in");
